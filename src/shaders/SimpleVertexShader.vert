...
void main() {
    ...
    gl_Position = vec4(
        (a_Position.x * cos(u_Angle) - a_Position.y * sin(u_Angle) + u_hexagonPosition.x) * scaleX * u_sizeScale,
        (a_Position.x * sin(u_Angle) + a_Position.y * cos(u_Angle) + u_hexagonPosition.x) * scaleY * u_sizeScale,
        a_Position.zw);
